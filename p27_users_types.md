...
Использование `type` не накладывает дополнительных гарантий безопасности типов. Это значит, что в функцию, котора принимает `Name` можно передать `Surname` или `String` и компилятор не заругает.
