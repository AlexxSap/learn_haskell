...
Хотя запрос значения переменой будет выполнен сразу, стоит помнить, что строки в haskell ленивы, потому лучше здесь использовать оператор `(!)` для того, чтобы прочитать результат сразу:
```haskell
!someValue <- lookupEnv "SOME"
```

Так же стоит помнить, что значение переменных среду всегда `String`, для преобразования в нужный тип можно использовать `Text.Read.readMaybe`:
```haskell
