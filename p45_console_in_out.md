...
readLn :: Read a => IO a
```
Читает строку из консоли и преобразовывает в нужный тип - это комбинация двух действий `getLine` и `read`. При использовании `readLn` нужно явно указать тип (аннотация типа), который мы хотим в резултате:
```haskell
num <- readLn :: IO Int 
```
