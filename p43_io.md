...
Haskell позволяет выполнять подобные панипуляции, но требует, чтобы они были помечены специально. Для этого существует параметризованный тип `IO`. Его можно сразу увидеть в фунцкии `main`:
```haskell
main :: IO ()
...
```
...
В примере выше мы выполняем действие чтения строки из консоли в переменную и далее передеём её в функцию `someFunc`. Но `getLine` возвращает `IO String`, а `someFunc` - чистая функция, которая принимает `String`. Проблема решается с помощью оператора `<-`, который извлекает значение из контекста `IO` и связывает его с именем паременной. Оператор `<-` работает только в блоке `do`. 
